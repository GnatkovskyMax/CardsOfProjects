<?php
function action_editManagerAjax(){
    if (is_auth()) {
            $formData = [
            ...
            if (empty($errors)) {
                ...
                if(editManager($formData,$formData['id'])){
                    $result = new_mysqli_fetch_all(getIdManager($formData['id']));
                ...
                renderView('dev/editManager', $errors);
            }
    }
}
function action_deletePhotoMeneger(){
    if (!empty($_POST['id'])){
        $id=+($_POST['id']);
...
function action_prices(){
    $prices= new_mysqli_fetch_all(all_price_for_product($_POST['id']));
    $id=$_POST['id'];
    renderView("dev/prices", ["prices" => $prices, "id"=>$id]);

}
function action_addPrice(){
    $from=$_POST['active_from'];
    $to=$_POST['active_to'];
    $price=$_POST['price'];
    $product_id=$_POST['product_id'];
    $res= addPrice($from, $to, $price, $product_id );
    if($res){
        $prices= new_mysqli_fetch_all(all_price_for_product($product_id));
        renderView("dev/prices", ["prices" => $prices, "id"=>$product_id]);
    }else{
        var_dump($_POST);
    }
}
function action_delPrice(){
    $product_id=$_POST['product_id'];
    $id=$_POST['id'];
    $res= delPrice($id);
    if($res){
...
function action_actyalPrice(){
    $prices= new_mysqli_fetch_all(all_price_for_product($_POST['id']));
    $id=$_POST['id'];
    $res= actyalPrice($id);

    if($res){
      echo  'Последняя добавленная цена: '.$res.'грн . ';
    ...
    for($i=0;$i<count($prices);$i++){
        if(strtotime($prices[$i]['active_from'])<= mktime()&& strtotime($prices[$i]['active_to'])>=mktime()){
            $actyal[$i]=[strtotime($prices[$i]['active_to'])-strtotime($prices[$i]['active_from'])];
            $all[strtotime($prices[$i]['active_to'])-strtotime($prices[$i]['active_from'])]=[$prices[$i]['price_id'], $prices[$i]['price'] ];
        }

    }

    asort($actyal);
    $new_actyal = array_values($actyal);
//    var_dump($all[$new_actyal[0][0]][0]);
    echo 'Актуальная цена на данный момент:'.($all[$new_actyal[0][0]][1]).'грн';
    echo ' Осуществил вывод по двум параметрам так как не понял какой важнее';
}
...
function action_allManagers(){
    $id = +($_POST['id']);
    $allId= new_mysqli_fetch_all(getAllManagerId($id));
    $allManagerProject = new_mysqli_fetch_all(allManagerById($allId));
    renderView('dev/managersAjax', ['managers' => $allManagerProject]);
}
function action_allProjects(){
    $id = +($_POST['id']);
    $allId= new_mysqli_fetch_all(getAllProjectId($id));
    $allProject = new_mysqli_fetch_all(allProjectById($allId));
    renderView('dev/projectsAjax', ['projects' => $allProject]);
}
...
function action_delPhotoManager(){
    if (!empty($_POST['id'])){
        $id=+($_POST['id']);
     $file= substr($_POST['file'], 1);
        $d = opendir("manager/$id");
        unlink("$file");
        closedir($d);
        $str='';
        $sql=" UPDATE `manager` SET `img`=\"$str\" WHERE `id`= $id";
        $res=insertUpdateDelate($sql);
        if($res){
                  echo '<br> <br> <br> <div class="alert-success" id="img">Фото удалено</div>';

        }
    }
}
function action_upPhotoManager(){
    require '/vendor/autoload.php';
    $id = $_POST['id'];
    $img= "/manager/$id/".time().$_FILES["max"]["name"];
    $name_files_general = "manager/$id/".time().$_FILES["max"]["name"];
    $imgManager = new_mysqli_fetch_all(imgManager($id));
    $o = substr($imgManager[0]['img'], 1);
    if(file_exists($o)){
        unlink($o);
    }
    ...
    $image->save
    ($name_files_general);//paste($watermark, $bottomRight)->
    $sql=" UPDATE `manager` SET `img`=\"$img\" WHERE `id`= $id";
    $res=insertUpdateDelate($sql);
    if($res){
        renderView('dev/wrappManagerPhoto', ['img' => $img, 'id'=> $id]);
    }else{
    }
